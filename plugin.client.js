// ============================================================================
// 会话产物侧栏插件 — Client 半部（artf-2 / pkg-23）
// 用途：作为 cordis_define 的 code.client 函数体。
// 安装方法见同目录 安装说明.md
// ============================================================================
return {
  apply(ctx) {
...
`))
    ...
    const PanelBody = (props) => {
      ...
      const load = (refresh) => {
        ...
        setLoading(true)
        setError(null)
        host.call('artifacts/current', { sessionId, refresh: !!refresh }).then((d) => {
      ...
      const doReveal = (f) => {
        ...
        host.call('artifacts/reveal', { path }).then((r) => {
      ...
      const doPreview = (f) => {
        ...
        setActivePv(f)
        host.call('artifacts/read', { path: f.abs || '' }).then((r) => {
          setPreview((prev) => {
            const next = {}
            for (const k in prev) next[k] = prev[k]
